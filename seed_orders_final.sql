...
-- Uses existing employees (IDs 1-8), clients (IDs 1-50), materials (IDs 1-71)
...
DECLARE
    ...
    v_manager_ids BIGINT[] := ARRAY[2, 2, 2];  -- manager (ID=2)
    v_workshop_ids BIGINT[] := ARRAY[1, 2, 3, 4, 5];
    v_employee_ids BIGINT[] := ARRAY[1, 2, 3, 4, 5, 6, 7, 8];
    i INTEGER;
    j INTEGER;
    k INTEGER;
    m INTEGER;
