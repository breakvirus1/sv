...
public class OrderAdminService {
    ...
    public List<Order> generateTestOrders(int count) {
        ...
        List<Long> clientIds = jdbcTemplate.query("SELECT id FROM svtables.clients WHERE deleted = false", (rs, rowNum) -> rs.getLong("id"));
        List<Long> employeeIds = jdbcTemplate.query("SELECT id FROM svtables.employees WHERE deleted = false", (rs, rowNum) -> rs.getLong("id"));
        List<MaterialInfo> materials = jdbcTemplate.query(
            "SELECT id, name, unit, price FROM svtables.materials WHERE deleted = false",
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int j = 0; j < itemCount; j++) {
                if (!materials.isEmpty()) {
                    ...
                    om.setWidth(BigDecimal.valueOf(1 + random.nextInt(5)));
                    om.setHeight(BigDecimal.valueOf(1 + random.nextInt(5)));
