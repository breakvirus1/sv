...
import { Box, Paper, Typography, TextField, Button, IconButton, Collapse, Input, Chip, Dialog } from '@mui/material';
...
const CommentsTab = ({ orderId, highlightCommentId, highlightReplyId }) => {
          ...
          {reply.images?.length > 0 && (
            <Box mb={1} display="flex" gap={1} flexWrap="wrap">
              {reply.images.map(img => (
                <img key={img.id} src={getImageUrl(img.url)} alt={img.originalName} style={{ maxWidth: 200, maxHeight: 200, borderRadius: 4, cursor: 'pointer' }} onClick={() => setPreviewImage(getImageUrl(img.url))} />
              ))}
            </Box>
          )}
            ...
            <Box sx={{ mt: 1, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              {(replyImagesMap[replyKey] || []).map(img => (
                <Box key={img.id} sx={{ position: 'relative', display: 'inline-block' }}>
                  <img src={getImageUrl(img.url)} alt={img.originalName} style={{ maxWidth: 100, maxHeight: 100, borderRadius: 4, cursor: 'pointer' }} onClick={() => setPreviewImage(getImageUrl(img.url))} />
                  <IconButton
                    size="small"
  ...
  return (
    <Box>
      ...
      <Dialog open={!!previewImage} onClose={() => setPreviewImage(null)} maxWidth="lg" fullWidth PaperProps={{ sx: { bgcolor: 'background.paper', m: 0 } }}>
        {previewImage && (
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 400, bgcolor: 'black', position: 'relative' }}>
            <IconButton onClick={() => setPreviewImage(null)} sx={{ position: 'absolute', top: 8, right: 8, color: 'white' }}>
              <Close />
            </IconButton>
            <img src={previewImage} alt="Превью" style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain' }} />
          </Box>
        )}
      </Dialog>
    </Box>
